...
Alias.prototype = {
   ...
   */
  afterCreatedRecord(record, opts, done) {
    if (!this.we) return done();
    const we = this.we;

    const aliasObj = record.$modelOptions.classMethods.urlAlias(record);
...
//
   ...
   * @param  {Function} done   callback
   */
  afterUpdatedRecord(record, opts, done) {
    if (!this.we) return done();
    const we = this.we;
   ...
   * @param  {Function} done   callback
   */
  afterDeleteRecord(record, opts, done) {
